"""
Weber Sprint — "ONE DAY LEFT" reel producer.

Builds a vertical 9:16 promo reel (1080x1920) from the real Weber "after"
photos already in public/gallery, plus two branded still cards. Crossfade
slideshow with a persistent brand bar + intro/outro cards, piped straight to
ffmpeg (no temp frames). Messaging reflects the final-day push: 24 of 30
cleaned, sprint ends Wed June 17.

Adapted from marketing/weber_reel.py to source photos from the repo.

Run from repo root: python3 marketing/weber_lastday_reel.py
"""
import math
import subprocess
...
# Real Weber afters from the repo gallery, with a short model note.
PHOTOS = [
    ("tsg-017-after.webp", "Weber Genesis II"),
    ("tsg-015-after.webp", "Weber Genesis II · 3-burner"),
    ("tsg-014-after.webp", "Weber · 4-burner + Sear Station"),
    ("tsg-016-cookbox-after.webp", "Weber Genesis · Sear Station"),
]
...
def load_upright(name):
    img = Image.open(GALLERY / name).convert("RGB")
...
def card_cinematic(photo, model, idx, headline):
    ...
    d = ImageDraw.Draw(c)
    # "LAST DAY" pill top-left
    pill = "LAST DAY · 15% OFF"
    d.rounded_rectangle([40, 40, 40 + d.textlength(pill, font=font(34)) + 44, 98], 29, fill=RED)
    d.text((62, 52), pill, font=font(34), fill=WHITE)
    # headline bottom
    ...
    p = OUT / f"weber-lastday-card-{idx}.png"
    c.save(p)
    return p
...
def reel_photo(photo, model):
    ...
    img.paste(Image.new("RGB", (RW, 230), NAVY), (0, RH - 230))
    ...
    # last-day pill top-left
    pill = "LAST DAY"
    d.rounded_rectangle([40, 60, 40 + d.textlength(pill, font=font(38)) + 48, 124], 32, fill=RED)
    d.text((64, 73), pill, font=font(38), fill=WHITE)
...
def build_reel(out_path):
    segs = [
        reel_card([("WEBER SPRINT", 130), ("ONE DAY LEFT", 150)],
                  sub="24 of 30 Webers cleaned — last call.", accent=RED),
    ]
    for name, model in PHOTOS:
        segs.append(reel_photo(load_upright(name), model))
    segs.append(
        reel_card([("BOOK YOUR", 96), ("WEBER", 200)],
                  sub="15% off · 30% with a neighbor · ends Wed, June 17", accent=RED)
    )
    FPS = 30
    HOLD = {0: 54, len(segs) - 1: 66}  # intro/outro hold longer
...
# ---------- run ----------
stills = []
heads = ["ANOTHER WEBER, BACK TO LIFE.", "GREASE GONE. SUMMER ON."]
stills.append(card_cinematic(load_upright(PHOTOS[0][0]), PHOTOS[0][1], 1, heads[0]).name)
stills.append(card_cinematic(load_upright(PHOTOS[2][0]), PHOTOS[2][1], 2, heads[1]).name)

reel = build_reel(OUT / "weber-lastday-reel.mp4")
